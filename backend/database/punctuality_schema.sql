...
CREATE TABLE IF NOT EXISTS realtime_delay_records (
    ...
    -- 数据来源和处理信息
    data_source TEXT DEFAULT 'GTFS_Realtime',
...
CREATE TABLE IF NOT EXISTS route_daily_punctuality (
    ...
    early_trips INTEGER DEFAULT 0,
    late_trips INTEGER DEFAULT 0,
    very_late_trips INTEGER DEFAULT 0,
    ...
    early_rate DECIMAL(5,2) DEFAULT 0,
    late_rate DECIMAL(5,2) DEFAULT 0,
    very_late_rate DECIMAL(5,2) DEFAULT 0,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    UNIQUE(region, route_id, stat_date),
    FOREIGN KEY (region) REFERENCES regions(region_id),
    FOREIGN KEY (region, route_id) REFERENCES routes(region, route_id)
);
...
CREATE TABLE IF NOT EXISTS stop_daily_punctuality (
    ...
    early_visits INTEGER DEFAULT 0,
    late_visits INTEGER DEFAULT 0,
    very_late_visits INTEGER DEFAULT 0,
    ...
    UNIQUE(region, stop_id, stat_date),
    FOREIGN KEY (region) REFERENCES regions(region_id),
    FOREIGN KEY (region, stop_id) REFERENCES stops(region, stop_id)
);
...
CREATE TABLE IF NOT EXISTS hourly_punctuality_stats (
    ...
    UNIQUE(region, route_id, stop_id, hour_of_day, stat_date),
    FOREIGN KEY (region) REFERENCES regions(region_id),
    FOREIGN KEY (region, route_id) REFERENCES routes(region, route_id),
    FOREIGN KEY (region, stop_id) REFERENCES stops(region, stop_id)
);
...
CREATE TABLE IF NOT EXISTS system_punctuality_overview (
    ...
    -- 线路排名（JSON格式存储）
    best_performing_routes JSON,
    worst_performing_routes JSON,

    -- 时段统计
    morning_peak_rate DECIMAL(5,2) DEFAULT 0,
    evening_peak_rate DECIMAL(5,2) DEFAULT 0,
    off_peak_rate DECIMAL(5,2) DEFAULT 0,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    UNIQUE(region, stat_date),
    FOREIGN KEY (region) REFERENCES regions(region_id)
);
...
-- 实时延误记录索引
CREATE INDEX IF NOT EXISTS idx_delay_records_region ON realtime_delay_records(region);
CREATE INDEX IF NOT EXISTS idx_delay_records_route_time ON realtime_delay_records(region, route_id, scheduled_time);
CREATE INDEX IF NOT EXISTS idx_delay_records_stop_time ON realtime_delay_records(region, stop_id, scheduled_time);
CREATE INDEX IF NOT EXISTS idx_delay_records_trip ON realtime_delay_records(region, trip_id);
...
-- 线路日统计索引
CREATE INDEX IF NOT EXISTS idx_route_daily_region ON route_daily_punctuality(region);
CREATE INDEX IF NOT EXISTS idx_route_daily_date ON route_daily_punctuality(region, stat_date);
CREATE INDEX IF NOT EXISTS idx_route_daily_route ON route_daily_punctuality(region, route_id);

-- 站点日统计索引
CREATE INDEX IF NOT EXISTS idx_stop_daily_region ON stop_daily_punctuality(region);
CREATE INDEX IF NOT EXISTS idx_stop_daily_date ON stop_daily_punctuality(region, stat_date);
CREATE INDEX IF NOT EXISTS idx_stop_daily_stop ON stop_daily_punctuality(region, stop_id);

-- 时段统计索引
CREATE INDEX IF NOT EXISTS idx_hourly_region ON hourly_punctuality_stats(region);
CREATE INDEX IF NOT EXISTS idx_hourly_date_hour ON hourly_punctuality_stats(region, stat_date, hour_of_day);
CREATE INDEX IF NOT EXISTS idx_hourly_route ON hourly_punctuality_stats(region, route_id);

-- 实时车辆位置索引
CREATE INDEX IF NOT EXISTS idx_vehicle_pos_region ON realtime_vehicle_positions(region);
CREATE INDEX IF NOT EXISTS idx_vehicle_pos_vehicle ON realtime_vehicle_positions(vehicle_id);
CREATE INDEX IF NOT EXISTS idx_vehicle_pos_trip ON realtime_vehicle_positions(region, trip_id);
...
-- 系统概览索引
CREATE INDEX IF NOT EXISTS idx_system_overview_region ON system_punctuality_overview(region);
CREATE INDEX IF NOT EXISTS idx_system_overview_date ON system_punctuality_overview(region, stat_date);
...
SELECT
    ...
    rdp.stat_date
FROM routes r
LEFT JOIN route_daily_punctuality rdp ON r.region = rdp.region AND r.route_id = rdp.route_id
...
SELECT
    ...
    sdp.stat_date
FROM stops s
LEFT JOIN stop_daily_punctuality sdp ON s.region = sdp.region AND s.stop_id = sdp.stop_id
...
-- 为相关表创建触发器
DROP TRIGGER IF EXISTS update_route_daily_punctuality_updated_at ON route_daily_punctuality;
CREATE TRIGGER update_route_daily_punctuality_updated_at
    BEFORE UPDATE ON route_daily_punctuality
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_stop_daily_punctuality_updated_at ON stop_daily_punctuality;
CREATE TRIGGER update_stop_daily_punctuality_updated_at
    BEFORE UPDATE ON stop_daily_punctuality
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_hourly_punctuality_stats_updated_at ON hourly_punctuality_stats;
CREATE TRIGGER update_hourly_punctuality_stats_updated_at
    BEFORE UPDATE ON hourly_punctuality_stats
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_system_punctuality_overview_updated_at ON system_punctuality_overview;
CREATE TRIGGER update_system_punctuality_overview_updated_at
    BEFORE UPDATE ON system_punctuality_overview
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_punctuality_config_updated_at ON punctuality_config;
